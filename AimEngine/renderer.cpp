...
void Renderer::Init() {
	...
	*/
	...
	// IBL: CREATE IRRADIANCE MAP
	Shader irradianceShader("Shaders/equirectangular.vert", "Shaders/irradiance.frag");
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_CUBE_MAP, cubemapTexture); // Bind the original skybox
	...
	// IBL: CREATE PREFILTER MAP
	Shader prefilterShader("Shaders/equirectangular.vert", "Shaders/prefilter.frag");
	...
	glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glGenerateMipmap(GL_TEXTURE_CUBE_MAP); // IMPORTANT: Enable mipmapping
...
void Renderer::DrawCube(const glm::mat4& model, const glm::mat4& view, const glm::mat4& projection) {
	...
	glBindTexture(GL_TEXTURE_2D, brdfLUTTexture);
	pbrShader->setInt("uIrradianceMap", 0);
	pbrShader->setInt("uPrefilterMap", 1);
	pbrShader->setInt("uBrdfLUT", 2);
